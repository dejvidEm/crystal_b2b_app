-- Exact requested time instead of morning/afternoon/flexible window
-- Run after 004_organization_min_vehicles.sql
...
begin
  if exists (
    ...
    set requested_time = case time_window::text
    ...
    update public.service_requests
    set requested_time = time '09:00'
    where requested_time is null;
  end if;
end $$;

alter table public.service_requests
  alter column requested_time set default time '09:00';
...
drop type if exists public.time_window;
...
begin
  ...
  if p_requested_time is null then
    raise exception 'Čas je povinný.';
  end if;
...
revoke all on function public.create_service_request(
  date, time, public.request_priority, text, jsonb
) from public;
grant execute on function public.create_service_request(
  date, time, public.request_priority, text, jsonb
) to authenticated;

-- Refresh PostgREST schema cache so the new RPC is visible immediately
